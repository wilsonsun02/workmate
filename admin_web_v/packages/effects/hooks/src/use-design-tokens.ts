...
export function useElementPlusDesignTokens() {
  ...
  const getCssVariableValue = (
    ...
    const result = isColor ? convertToRgb(`hsl(${value})`) : value;
    if (isColor && result.replace(/\s/g, '') === 'rgba(0,0,0,0)') {
  ...
  watch(
    ...
    () => {
      ...
      const variables: Record<string, string> = {
        ...
        '--el-bg-color-overlay': getCssVariableValue('--popover', true, background),
        '--el-bg-color-page': getCssVariableValue('--background-deep', true, background),
